...
func _ready():
	...
	progreso = get_node("speed/progreso")
...
func set_progress_position(value):
	progress_position = clamp(value, 770, 1180)
	
func calculate_progress():
	progress_percent = main.current_distance * 100 / main.distance_to_goal
	set_progress_position((progress_percent * 410 / 100) + 770)
	progreso.position.x = progress_position
